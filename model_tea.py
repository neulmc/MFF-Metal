...
class Init_Block(nn.Module):
    def __init__(self, tiny_c = 32):
        ...
        self.init_conv = nn.Sequential(
            Conv(3, tiny_c, 3, 2, padding=1, bn_acti=True),
...
class MAD(nn.Module):
    def __init__(self, c1=16, c2=32, classes=19, tiny_c=32):
        super(MAD, self).__init__()
        self.c1, self.c2 = c1, c2
        self.LMFFNet_Block_2 = nn.Sequential()

        self.mid_layer_1x1 = Conv(tiny_c*4 + 3, c1, 1, 1, padding=0, bn_acti=False)

        self.deep_layer_1x1 = Conv(tiny_c*8 + 3, c2, 1, 1, padding=0, bn_acti=False)
        ...
        self.DwConv2 = Conv(tiny_c*8 + 3, tiny_c*8 + 3, (3, 3), 1, padding=(1, 1), groups=tiny_c*8 + 3, bn_acti=True)
        self.PwConv2 = Conv(tiny_c*8 + 3, classes, 1, 1, padding=0, bn_acti=False)
...
class self_net(nn.Module):
    def __init__(self, classes=4, block_1=3, block_2=5, tiny_c = 28):
        super().__init__()
        self.classes = classes
        self.block_1 = block_1
        self.block_2 = block_2
        self.Init_Block = Init_Block(tiny_c)
        ...
        self.FFM_A = FFM_A(tiny_c + 3)

        self.downsample_1 = DownSamplingBlock(tiny_c + 3, tiny_c*2)

        self.SEM_B_Block1 = SEM_B_Block(num_channels=tiny_c*2, num_block=self.block_1, dilation=[2, 2, 2], flag=1)

        self.FFM_B1 = FFM_B(ch_in=tiny_c*4 + 3, ch_pmca=tiny_c*2)

        self.downsample_2 = DownSamplingBlock(tiny_c*4 + 3, tiny_c*4)

        self.SEM_B_Block2 = SEM_B_Block(num_channels=tiny_c*4, num_block=self.block_2, dilation=[4, 4, 8, 8, 16, 16, 32, 32], flag=2)

        self.FFM_B2 = FFM_B(ch_in=tiny_c*8 + 3, ch_pmca=tiny_c*4)

        self.MAD = MAD(classes=self.classes, tiny_c=tiny_c)
        self.softmax = nn.Softmax()

    def forward(self, input, T_KD=False):
        # Init Block
