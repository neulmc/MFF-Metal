...
class DecoderBottleneck(nn.Module):
    ...
    def forward(self, x, x_concat=None):
        ...
        if x_concat is not None:
            diffY = x_concat.size()[2] - x.size()[2]
            diffX = x_concat.size()[3] - x.size()[3]
...
class Encoder(nn.Module):
    def __init__(self, img_dim, in_channels, out_channels, head_num, mlp_dim, block_num, patch_dim):
        super().__init__()
        #self.conv_adjust = nn.Conv2d(in_channels=1344, out_channels=1664, kernel_size=1, stride=1, padding=0)
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=8, stride=2, padding=3, bias=False)
        ...
        self.vit_img_dim = int(np.ceil(img_dim / patch_dim))
